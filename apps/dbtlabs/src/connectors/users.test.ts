...
describe('users connector', () => {
  describe('getUsers', () => {
    beforeEach(() => {
      server.use(
        http.get(`https://cloud.getdbt.com/api/v2/accounts/${accountId}/users`, ({ request }) => {
          if (request.headers.get('Authorization') !== `Bearer ${personalToken}`) {
    ...
    test('should return users and nextPage when the token is valid and their is another page', async () => {
      await expect(
        getUsers({ personalToken, accountId, dbtRegion, afterToken: nextCursor })
    ...
    test('should return users and no nextPage when the token is valid and their is no other page', async () => {
      await expect(
        getUsers({ personalToken, accountId, dbtRegion, afterToken: null })
      ).resolves.toStrictEqual({
        validUsers,
        invalidUsers,
        nextPage: null,
