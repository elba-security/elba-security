...
describe('rate-limit middleware', () => {
  ...
  test('should not transform the output when the error is not about Auth0 rate limit', () => {
    expect(
      rateLimitMiddleware
        .init()
        // @ts-expect-error -- this is a mock
  ...
  test('should transform the output error to RetryAfterError when the error is about Auth0 rate limit', () => {
    const rateLimitReset = '1700137003';

    const rateLimitError = new Auth0Error('foo bar', {
      response: {
        // @ts-expect-error -- this is a mock
