...
describe('users connector', () => {
  describe('getUsers', () => {
    ...
    beforeEach(() => {
      server.use(
        http.get(`${env.CALENDLY_API_BASE_URL}/users`, ({ request }) => {
    ...
    test('should throws when the token is invalid', async () => {
      await expect(getUsers({ accessToken: 'foo-bar' })).rejects.toBeInstanceOf(CalendlyError);
    });
  });
    ...
    beforeEach(() => {
      server.use(
        http.post<{ userId: string }>(
          `${env.CALENDLY_API_BASE_URL}/workspaces/:workspaceId/removeUser`,
          ({ request }) => {
            if (request.headers.get('Authorization') !== `Bearer ${validToken}`) {
              return new Response(undefined, { status: 401 });
            }
