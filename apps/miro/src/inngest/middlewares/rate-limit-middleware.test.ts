...
describe('rate-limit middleware', () => {
  ...
  test('should not transform the output when the error is not about Miro rate limit', () => {
    expect(
      rateLimitMiddleware
        .init()
        // @ts-expect-error -- this is a mock
  ...
  test('should transform the output error to RetryAfterError when the error is about Miro rate limit', () => {
    // Mock current time
    const currentTime = Math.floor(Date.now() / 1000);
    const resetTime = currentTime + 30; // Simulate reset happening in 30 seconds

    const rateLimitError = new MiroError('Rate limit exceeded', {
      // @ts-expect-error this is a mock
      response: {
        status: 429,
