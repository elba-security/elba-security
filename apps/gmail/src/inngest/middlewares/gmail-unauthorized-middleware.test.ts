...
import { gmailUnauthorizedMiddleware } from './gmail-unauthorized-middleware';

describe('gmail-unauthorized-middleware', () => {
  test('should not transform the output when there is no error', async () => {
    const send = vi.fn().mockResolvedValue(undefined);

    const result = await gmailUnauthorizedMiddleware
