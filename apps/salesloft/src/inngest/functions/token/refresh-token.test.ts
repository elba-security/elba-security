...
const setup = createInngestFunctionMock(refreshToken, 'salesloft/token.refresh.requested');
...
describe('refresh-token', () => {
  ...
  test('should update encrypted tokens and schedule the next refresh', async () => {
    ...
    if (!updatedOrganisation) {
      throw new SalesloftError(`Organisation with ID ${organisation.id} not found.`);
    }
