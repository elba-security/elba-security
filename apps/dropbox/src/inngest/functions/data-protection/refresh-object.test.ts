...
vi.mock('@/connectors/dropbox/dbx-files', async () => {
  ...
  return {
    ...
    DBXFiles: vi.fn(() => {
      return {
        ...
        fetchFoldersMetadataMembersAndMapDetails:
          mocks.fetchFoldersMetadataMembersAndMapDetailsMock,
      };
    }),
  };
});
...
describe('refreshObject', () => {
  ...
  test('should successfully refresh the requested folder', async () => {
    const elba = spyOnElba();
    mocks.fetchSharedLinksByPathMock.mockResolvedValueOnce([sharedLinks.at(0), sharedLinks.at(1)]);
    mocks.fetchFolderOrFileMetadataByPathMock.mockResolvedValueOnce(foldersAndFiles.at(0));
    mocks.fetchFoldersMetadataMembersAndMapDetailsMock.mockResolvedValueOnce([
      {
        id: 'id:folder-id-1',
        metadata: {
          is_personal: true,
  ...
  test('should successfully refresh the requested file', async () => {
    const elba = spyOnElba();
    mocks.fetchSharedLinksByPathMock.mockResolvedValueOnce([sharedLinks.at(0), sharedLinks.at(1)]);
    mocks.fetchFolderOrFileMetadataByPathMock.mockResolvedValueOnce(foldersAndFiles.at(2));
    mocks.fetchFilesMetadataMembersAndMapDetailsMock.mockResolvedValueOnce([
      {
        id: 'id:file-id-1',
        metadata: {
          is_personal: true,
