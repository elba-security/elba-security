...
describe('unauthorized middleware', () => {
  ...
  test('should not transform the output when the error is not about Livestorm authorization', async () => {
    const send = vi.fn().mockResolvedValue(undefined);
    await expect(
      unauthorizedMiddleware
        // @ts-expect-error -- this is a mock
  ...
  test('should transform the output error to NonRetriableError and remove the organisation when the error is about Livestorm authorization', async () => {
    const unauthorizedError = new LivestormError('foo bar', {
      // @ts-expect-error this is a mock
      response: {
        status: 401,
      },
    ...
    expect(send).toBeCalledWith({
      name: 'livestorm/app.uninstalled',
      data: {
        organisationId: organisation.id,
      },
    });
