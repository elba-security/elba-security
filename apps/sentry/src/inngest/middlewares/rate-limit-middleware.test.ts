...
describe('rate-limit middleware', () => {
  ...
  test('should not transform the output when the error is not about sentry rate limit', async () => {
    await expect(
      rateLimitMiddleware
        .init()
        // @ts-expect-error -- this is a mock
  ...
  test('should transform the output error to RetryAfterError when the error is about sentry rate limit', async () => {
    const rateLimitError = new SentryError('foo bar', {
