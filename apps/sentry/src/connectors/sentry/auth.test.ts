...
describe('auth connector', () => {
  describe('getToken', () => {
    beforeEach(() => {
      server.use(
        http.post(`${env.SENTRY_APP_INSTALL_URL}oauth_token`, async ({ request }) => {
    ...
    test('should throw when the code is invalid', async () => {
      await expect(getToken('wrong-code')).rejects.toBeInstanceOf(SentryError);
    });
  });
  ...
  describe('getRefreshToken', () => {
    ...
    test('should throw when the refreshToken is invalid', async () => {
      await expect(getToken('wrong-refreshtoken')).rejects.toBeInstanceOf(SentryError);
    });
  });
  ...
  describe('getWorkspaceIds', () => {
    beforeEach(() => {
      server.use(
        http.get(`${env.SENTRY_API_BASE_URL}/workspaces`, ({ request }) => {
    ...
    test('should throw when the accessToken is invalid', async () => {
      await expect(getWorkspaceIds(invalidToken)).rejects.toBeInstanceOf(SentryError);
    });
  });
});
