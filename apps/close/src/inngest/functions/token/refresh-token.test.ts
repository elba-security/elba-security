...
import * as authConnector from '@/connectors/close/auth';
...
const setup = createInngestFunctionMock(refreshToken, 'close/token.refresh.requested');
...
describe('refresh-token', () => {
  ...
  test('should update encrypted tokens and schedule the next refresh', async () => {
    ...
    if (!updatedOrganisation) {
      throw new CloseError(`Organisation with ID ${organisation.id} not found.`);
    }
    ...
    expect(step.sendEvent).toBeCalledWith('next-refresh', {
      name: 'close/token.refresh.requested',
      data: {
        organisationId: organisation.id,
