...
describe('auth connector', () => {
  describe('getToken', () => {
    beforeEach(() => {
      server.use(
        http.post(`${env.CLOSE_API_BASE_URL}/oauth2/token`, async ({ request }) => {
    ...
    test('should return the accessToken when the code is valid', async () => {
      await expect(getToken(validCode)).resolves.toStrictEqual({
        ...
        expiresIn,
        organizationUri: organizationId,
      });
    });

    test('should throw when the code is invalid', async () => {
      await expect(getToken('wrong-code')).rejects.toBeInstanceOf(CloseError);
    });
  });
  ...
  describe('getRefreshToken', () => {
    ...
    test('should throw when the refreshToken is invalid', async () => {
      await expect(getRefreshToken('wrong-refreshtoken')).rejects.toBeInstanceOf(CloseError);
    });
  });
});
