...
describe('rate-limit middleware', () => {
  ...
  test('should not transform the output when the error is not about Harvest rate limit', () => {
    expect(
      rateLimitMiddleware
        .init()
        // @ts-expect-error -- this is a mock
  ...
  test('should transform the output error to RetryAfterError when the error is about Harvest rate limit', () => {
    const rateLimitReset = '1700137003';

    const rateLimitError = new HarvestError('foo bar', {
      response: {
        // @ts-expect-error -- this is a mock
        headers: { 'Retry-After': rateLimitReset },
      },
      // @ts-expect-error -- this is a mock
