...
describe('auth connector', () => {
  describe('getToken', () => {
    beforeEach(() => {
      server.use(
        http.post(`${env.HARVEST_APP_INSTALL_URL}/token`, async ({ request }) => {
  ...
  describe('getWorkspaceIds', () => {
    beforeEach(() => {
      server.use(
        http.post(`${env.HARVEST_API_BASE_URL}`, ({ request }) => {
    ...
    test('should throw when the accessToken is invalid', async () => {
      await expect(getWorkspaceIds(invalidToken)).rejects.toBeInstanceOf(HarvestError);
    });
  });
});
