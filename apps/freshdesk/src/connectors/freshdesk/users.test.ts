...
describe('users connector', () => {
  describe('getUsers', () => {
    beforeEach(() => {
      server.use(
        http.get(`${env.FRESHDESK_API_BASE_URL}/users`, ({ request }) => {
    ...
    test('should throws when the token is invalid', async () => {
      await expect(getUsers({ accessToken: 'foo-bar' })).rejects.toBeInstanceOf(FreshdeskError);
    });
  });
    ...
    beforeEach(() => {
      server.use(
        http.delete<{ userId: string }>(
          `${env.FRESHDESK_API_BASE_URL}/users/${userId}`,
          ({ request }) => {
            if (request.headers.get('Authorization') !== `Bearer ${validToken}`) {
              return new Response(undefined, { status: 401 });
            }
