...
import { BitbucketError } from '@/connectors/commons/error';
 ...
 * The middleware checks for specific errors (instances of BitbucketError) that indicate a rate limit has been reached,
 ...
 * - Handles BitbucketError, specifically looking for a 'Retry-After' header in the error response.
      ...
      onFunctionRun: ({ fn }) => {
        return {
          transformOutput: (ctx) => {
            ...
            const retryAfter =
              error instanceof BitbucketError && error.response?.headers.get('Retry-After');
