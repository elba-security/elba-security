...
describe('auth connector', () => {
  describe('getAccessToken', () => {
    ...
    beforeEach(() => {
      server.use(
        http.post(env.BB_ACCESS_TOKEN_URL, async ({ request }) => {
    ...
    test('should throw when the accessCode is invalid', async () => {
      await expect(getAccessToken('wrong-code')).rejects.toBeInstanceOf(BitbucketError);
    });
  });
  ...
  describe('refreshAccessToken', () => {
    ...
    test('should throw when the refreshToken is invalid', async () => {
      await expect(refreshAccessToken('wrong-token')).rejects.toBeInstanceOf(BitbucketError);
    });
  });
});
