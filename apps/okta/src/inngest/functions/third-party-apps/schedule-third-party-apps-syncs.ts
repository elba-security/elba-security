...
export const scheduleThirdPartyAppsSync = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    if (invalidOrganisations.length) {
      logger.error('Failed to schedule third-party apps sync due to missing nango connection ID', {
        invalidOrganisations,
      });
      throw new NonRetriableError(
        'Failed to schedule third-party apps sync due to missing nango connection ID'
      );
    }
