...
import { LinearError } from '@/connectors/commons/error';
 ...
 * The middleware checks for specific errors (instances of LinearError) that indicate a rate limit has been reached,
 ...
 * - Handles LinearError, specifically looking for a 'Retry-After' header in the error response.
      ...
      onFunctionRun: ({ fn }) => {
        ...
        return {
          transformOutput: (ctx) => {
            ...
            const retryAfter =
              error instanceof LinearError && error.response?.headers.get('Retry-After');
            ...
            return {
              ...
              result: {
                ...
                error: new RetryAfterError(
                  `Linear rate limit reached by '${fn.name}'`,
                  retryAfter,
                  {
                    cause: error,
                  }
